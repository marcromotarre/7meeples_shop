...
import NewnessByMonth from "./newness-by-month";
...
export default function NewnessMain() {
  ...
  const get_newness_boardgames_info = () => {
    if (boardgames.length > 0 && newness.length > 0) {
      const completeNewnessData = newness.map(
        ({ month, year, boardgames: boardagmes_ids }) => {
          return {
            year,
            month,
  ...
  if (newnessCompleteInfo.length > 0) {
    console.log(newnessCompleteInfo, newnessCompleteInfo[0]);
  }
  ...
  return (
    ...
    >
      ...
      {!loading &&
        newnessCompleteInfo.map(({ year, month, boardgames }) => (
          <NewnessByMonth
            year={year}
            month={month}
